...
class File:
    ...
    def filename_to_id(self):
        filename_ascii = re.sub("[^0-9a-zA-Z_-]", "_", self.filename())[:50]  # Truncate to keep it short
        raw = self.filename()
        if self.acls:
            raw += str(self.acls)
        # filename_hash = hashlib.sha256(raw.encode("utf-8")).hexdigest()
        characters = string.ascii_letters + string.digits  # a-zA-Z0-9
        random_str = ''.join(random.choices(characters, k=20))
        return f"file-{filename_ascii}-{random_str}"
    ...
    #     acls_hash = ""
    #     if self.acls:
    #         acls_hash = base64.b16encode(str(self.acls)[:36].encode("utf-8")).decode("ascii")
