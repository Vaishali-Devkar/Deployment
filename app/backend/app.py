...
async def upload(auth_claims: dict[str, Any]):
    ...
    group_access = form_data.get('groupAccess')
    ...
    user_oid = auth_claims["oid"]
    user_groups = auth_claims["groups"]

    # logging.warning(f"GROUP_IDS:{user_groups}")

    # logging.warning(f"Devesh: {auth_claims}")
    ...
    if group_access == "true":
        filename = f"grp__{filename}"  # <-- Append prefix if group access is true
    file_io = file
...
async def list_uploaded(auth_claims: dict[str, Any]):
    ...
    try:
        ...
        async for path in all_paths:
           ...
           is_group = filename.startswith("grp__")
        #    clean_name = filename[7:] if is_group else filename
           clean_name = filename.replace("grp__", "") if is_group else filename
